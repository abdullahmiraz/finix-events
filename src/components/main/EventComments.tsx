...
const comments = [
    {
        ...
        color: "bg-[#357863] text-white",
        avatarBg: "bg-[#f7e9d0] text-[#357863]",
        replies: [
            {
                id: 2,
                name: "Rifat Khan",
                ...
                color: "bg-[#f7e9d0] text-[#357863]",
                avatarBg: "bg-[#357863] text-[#f7e9d0]",
            },
            {
                id: 3,
                name: "Rifat Khan",
        ...
        <div className={`rounded-xl p-4 flex flex-col gap-1 ${comment.color}`}>
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <span className={`rounded-full w-9 h-9 flex items-center justify-center font-bold text-sm ${comment.avatarBg}`}>
                        {comment.initials}
                    </span>
                    <span className="font-medium">
                        {comment.name}
                        {comment.isHost && (
                            <span className="text-xs ml-1 text-[#f7e9d0]">(Host)</span>
                        )}
                    </span>
                </div>
            ...
            {comments.map((comment) => (
                <div key={comment.id} className="flex flex-col gap-1">
                    ...
                    <div className="ml-14 mt-1">
                        <button className="text-sm text-gray-500 hover:underline mb-2 flex items-center gap-1"> <CornerDownRight className="w-4 h-4" /> Reply</button>
                        {comment.replies &&
